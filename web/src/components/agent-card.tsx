...
function getListingStatus(createdAt: string): { color: string; label: string } {
  const now = Date.now();
  const created = new Date(createdAt).getTime();
  const daysSince = (now - created) / (1000 * 60 * 60 * 24);
  if (daysSince < 14) return { color: 'bg-emerald-400', label: 'New listing' };
  if (daysSince < 90) return { color: 'bg-sky-400', label: 'Recent listing' };
  return { color: 'bg-zinc-500', label: 'Listed' };
}
...
export default function AgentCard({ agent, index = 0 }: AgentCardProps) {
  ...
  const listing = getListingStatus(agent.created_at);
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                ...
                <div className="inline-flex items-center gap-1.5 shrink-0 rounded-full border border-white/[0.06] bg-white/[0.03] px-2 py-0.5">
                  <div className={`w-2 h-2 rounded-full ${listing.color} shadow-[0_0_6px_currentColor]`} />
              ...
              <div className="flex flex-wrap items-center gap-1.5 mt-2">
                {agent.platforms && agent.platforms.length > 0 && (
                  <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-mono text-zinc-500 bg-white/[0.04] border border-white/[0.06]">
                    {agent.platforms.length} linked platform{agent.platforms.length !== 1 ? 's' : ''}
                  </span>
                )}
                ...
                {agent.capabilities && agent.capabilities.length > 2 && (
                  <span className="text-[9px] text-zinc-600 font-mono">+{agent.capabilities.length - 2} more</span>
                )}
              </div>
            </div>
